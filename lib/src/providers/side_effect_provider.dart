...
import 'package:jnap/src/models/device_info.dart';
...
//TODO: Need a way to access the serial number
class SideEffectNotifier extends Notifier<JNAPSideEffect> {
  ...
  //TODO: Have changed the way to get the serial number
  Future<(bool, JNAPResult?)> testRouterReconnected(String serialNumber) async {
    // final pref = await SharedPreferences.getInstance();
    // final cachedSerialNumber =
    //     pref.getString(pCurrentSN) ?? pref.getString(pPnpConfiguredSN);

    return _getDeviceInfo()
        .then((devceInfo) => devceInfo.serialNumber == serialNumber)
        .then((value) async =>
            (value ? await testRouterFullyBootedUp() : (false, null)))
        .onError((error, stackTrace) => (false, null));
  }

  //TODO: Wait for cache implementation: fetchRemote: true, .noCache
  Future<RouterWANStatus> _getWANStatus() => Jnap.instance
      .send(
        action: GetWANStatus.instance,
        overrides: JNAPConfigOverrides(timeoutMs: 3000, retries: 0),
      )
      .then((response) => RouterWANStatus.fromMap(response.output));

  //TODO: Wait for cache implementation: fetchRemote: true, .noCache
  Future<NodeDeviceInfo> _getDeviceInfo() => Jnap.instance
      .send(
