...
class EmailRule extends RegExValidationRule {
  ...
  RegExp get _rule => RegExp(
      r"^[a-zA-Z0-9.!#$%&‘*+/=?^_`{|}~-]+@([a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z0-9]{2,}$");
}
...
// This is to check the invalid characters in the host name
class HostNameCheckRule extends RegExValidationRule {
  @override
  String get name => 'HostNameCheckRule';
...
class SubnetMaskRule extends ValidationRule {
  ...
  bool validate(String input) {
    try {
      return IpAddressUtils.isValidSubnetMask(
        input,
...
class HostValidForGivenRouterIPAddressAndSubnetMaskRule extends ValidationRule {
  ...
  @override
  bool validate(String input) {
    final hostIPAddressNum = IpAddressUtils.ipToNum(input);
    final routerIPAddressNum = IpAddressUtils.ipToNum(routerIPAddress);
    final subnetMaskNum = IpAddressUtils.ipToNum(subnetMask);
  ...
  bool _isIPValidForSubnet(int ipAddressNum, int subnetMaskNum) {
    final networkIdNum = (ipAddressNum & subnetMaskNum) & 0xFFFFFFFF;
    final invertedSubnetMask = (~subnetMaskNum) & 0xFFFFFFFF;
    final broadcastIdNum = (networkIdNum | invertedSubnetMask) & 0xFFFFFFFF;
